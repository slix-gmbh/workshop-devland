...
- pyroscope und beyla autoinstrumenting mit Alloy...

## Dateien erstellen

In Verzeichnis 08_pyroscope wechseln. Dann folgende Dateien erstellen:
...
commonLabels:
    app.kubernetes.io/part-of: pyroscope
...
configMapGenerator:
  - name: pyroscope-dashboards
    files:
      - dashboards/pyroscope-reads.json
      - dashboards/pyroscope-writes.json
    options:
      labels:
        grafana_dashboard: "1"
        app.kubernetes.io/part-of: pyroscope
...
Dann per helm Pyroscope installieren
```
helm -n pyroscope upgrade --install pyroscope grafana/pyroscope -f pyroscope-values.yaml
```
...
```
Release "pyroscope" does not exist. Installing it now.
NAME: pyroscope
LAST DEPLOYED: Sun Jun 23 01:00:38 2024
NAMESPACE: pyroscope
STATUS: deployed
REVISION: 1
TEST SUITE: None
...
pyroscope-0   1/1     Running   0          102s
```

Und noch die Dashboards installieren:
```
kubectl apply -k .
```
